...
export default function EducacaoArtisticaECulturalScreen() {
  ...
  useEffect(() => {
    const carregarConteudo = async () => {
      try {
        const docRef = doc(db, 'servicos', 'educacaoArtisticaECultural');
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          setConteudo(docSnap.data() as any);
        } else {
          console.warn('Documento "educacaoArtisticaECultural" não encontrado.');
        }
      } catch (error) {
        ...
        <View>
          ...
          {user && (
            <TouchableOpacity
              ...
              onPress={() =>
                router.push({
                  pathname: '/(stacks)/editarEducacaoArtistica',
                  params: {
