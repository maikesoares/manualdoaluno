...
export default function ProgramaRenovarScreen() {
  ...
  useEffect(() => {
    async function carregarConteudo() {
      try {
        const docRef = doc(db, 'servicos', 'programaRenovar');
        ...
        if (docSnap.exists()) {
          ...
          console.warn('Documento "programaRenovar" não encontrado.');
        }
      } catch (error) {
    ...
    <ScrollView contentContainerStyle={servicosStyle.container}>
      <View style={servicosStyle.card}>
        <View style={servicosStyle.header}>
          <FontAwesome name="refresh" size={40} color={tintColorWhite} />
        ...
        {user && (
          <TouchableOpacity
            ...
            onPress={() =>
              router.push({
                pathname: '/(stacks)/editarProgramaRenovar',
                params: {
