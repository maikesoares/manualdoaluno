...
export default function EducacaoDiversidadeScreen() {
  ...
  useEffect(() => {
    const carregarConteudo = async () => {
      try {
        const docRef = doc(db, 'servicos', 'educacaoDiversidade');
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          setConteudo(docSnap.data() as any);
        } else {
          console.warn('Documento "educacaoDiversidade" não encontrado.');
        }
      } catch (error) {
        ...
        <View>
          ...
          {user && (
            <TouchableOpacity
              ...
              onPress={() =>
                router.push({
                  pathname: '/(stacks)/editarDiversidade',
                  params: {
