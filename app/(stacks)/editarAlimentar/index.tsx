...
export default function EditarSegurancaScreen() {
  const router = useRouter();
  const { title, texto, subText, subText1, subText2, download } = useLocalSearchParams();
  ...
  const salvarAlteracoes = async () => {
    try {
      const docRef = doc(db, 'servicos', 'segurancaAlimentar');
    ...
    <KeyboardAvoidingView
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <View style={styles.container}>
          ...
          <Text style={styles.label}>Subtexto 1</Text>
          <TextInput
            style={styles.input}
            value={novoSubText}
          ...
          <Text style={styles.label}>Subtexto 3</Text>
          <TextInput
            style={styles.input}
            value={novoSubText2}
            onChangeText={setNovoSubText2}
            multiline
          />
