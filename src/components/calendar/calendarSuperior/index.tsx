...
const CalendarAcademicoSuperior: React.FC = () => {
  const [markedDates, setMarkedDates] = useState<Record<string, MarkedDate>>({});
  const [eventos, setEventos] = useState<Evento[]>([]);
  const [mesAtual, setMesAtual] = useState<string>(new Date().toISOString().slice(0, 7));
  ...
  async function carregarEventos() {
    const querySnapshot = await getDocs(collection(db, 'eventosAcademicos'));
  ...
  const salvarEvento = async () => {
    ...
    await setDoc(doc(db, 'eventosAcademicos', evento.data), evento);
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            await deleteDoc(doc(db, 'eventosAcademicos', eventoData.trim()));
            setModalVisible(false);
            carregarEventos();
          },
        },
...
export { CalendarAcademicoSuperior };
